  ...
  if (registeredEmail) {
    return (
      <div className="fixed inset-0 z-1000 grid place-items-center bg-black/55 p-4" onMouseDown={onClose}>
        ...
        >
          <button className="absolute right-0 top-0 min-h-11 bg-brand-dark px-4 py-3 font-mono text-xs font-black uppercase tracking-widest text-white" type="button" onClick={onClose} aria-label="Cerrar">
            Cerrar
          </button>
      ...
      >
        ...
        <div className="p-8 pb-6">
          ...
          <form onSubmit={handleSubmit} className="grid gap-4">
            ...
            <button
              type="submit"
              className="mt-2 inline-flex min-h-11 min-w-0 w-full items-center justify-center rounded-xl border border-brand-dark bg-brand-dark px-4 py-3 font-mono text-xs font-black uppercase tracking-widest text-white no-underline transition hover:bg-brand-darker hover:border-brand-darker disabled:cursor-not-allowed disabled:opacity-45"
              disabled={submitting}
            >
              {submitting
                ? "Procesando…"
